...
func (suite *RepositorySuite) TestPrepareUpdateFields() {
	suite.Run("with all fields populated", func() {
		// Setup a complete entity
		name := "Test Client"
		email := "test@example.com"
		stage := "registration"
		source := "website"
		...
		// Execute
		result := prepareUpdateFields(clientData)

		// Verify - compare with the actual values, not pointers
		suite.Equal(&name, result["name"])
		suite.Equal(&email, result["email"])
		suite.Equal(&stage, result["current_stage"])
		suite.Equal(&source, result["source"])
		suite.Equal(&channel, result["channel"])
		suite.Equal(&app, result["app"])
		suite.Equal(&now, result["last_login"])
		suite.Equal(isActive, result["is_active"])
		suite.NotNil(result["last_updated"])
	})

	suite.Run("with minimal fields", func() {
		name := "Test Client"
		...
		// Verify - use the actual values
		suite.Equal(&name, result["name"])
		suite.Equal(true, result["is_active"]) // Default value
		suite.NotNil(result["last_updated"])
		suite.Nil(result["email"])
		suite.Nil(result["current_stage"])
		suite.Nil(result["source"])
		suite.Nil(result["channel"])
		suite.Nil(result["app"])
		suite.Nil(result["last_login"])
	})

	suite.Run("with inactive client", func() {
		// Setup with isActive = false
		isActive := false
		clientData := client.Entity{
			IsActive: &isActive,
		}
		...
		// Verify
		suite.Equal(false, result["is_active"])
	})
}

func (suite *RepositorySuite) TestPrepareArgs() {
	clientRepo := suite.repository.(*ClientRepository)
	...
	suite.Run("with all fields populated", func() {
		...
		// Execute
		result := clientRepo.prepareArgs(clientData)

		// Verify - comparing pointers to pointers
		suite.Equal(&name, result["name"])
		suite.Equal(&email, result["email"])
		suite.Equal(&stage, result["current_stage"])
		suite.Equal(&source, result["source"])
		suite.Equal(&channel, result["channel"])
		suite.Equal(&isActive, result["is_active"])
		suite.Equal(&now, result["registration_date"])
		suite.Equal(&now, result["last_updated"])
	})

	suite.Run("with empty entity", func() {
	...
	suite.Run("with partial fields", func() {
		// Setup with only some fields
		name := "Partial Client"
		isActive := true
